...
def read_rip_file(file, object_name, tex_path):
    ...
    (
        ...
        shaders_count,
        attributes_count,
    ) = struct.unpack("LLLLLL", file.read(24))

    # read in vertex attributes
    for x in range(attributes_count):
        semantic = read_string(file)
        semantic_index, offset, size, type_map_elements = struct.unpack(
            "LLLL", file.read(16)
        )
    ...
    for x in range(vertex_count):
        ...
        for y in range(len(vertex_attrib_types_array)):
            ...
            if y == posx_idx:
                vx = attrib_value * g_scale
            if y == posy_idx:
                vy = attrib_value * g_scale
            if y == posz_idx:
                vz = attrib_value * g_scale
